...
adminRouter.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    if (
      ...
      res
        .status(403)
        .json({ error: "You do not have permission to authorize users!" });
      return;
    }
    ...
    if (process.env.BOT_TOKEN !== undefined) {
      ...
      try {
        ...
        console.error(e);
        console.warn(
          `Warning: Discord ID ${req.params["discordId"]} could not be validated with the Discord API!`,
        );
        res.status(400).json({ error: "Failed to validate Discord ID!" });
        return;
      }

      await setPermissionRole(
...
adminRouter.get("/disable-user/:discordId", async (req, res: Response) => {
  if (
    ...
    res
      .status(403)
      .json({ error: "You do not have permission to disable users!" });
    return;
  }
