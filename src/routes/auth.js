...
if (TOKEN_EXPIRATION_TIME === "-1") {
  console.warn(
    "Warning: TOKEN_EXPIRATION_TIME is set to -1, which means authentication tokens will never expire!",
  );
}
...
router.post("/login", async (req, res) => {
  ...
  if (req.body.code === "DEMO_USER") {
    if (!DEMO_USER_ENABLED) {
      res.status(403).json({
        error: "Demo user is not enabled",
        error_code: "DEMO_USER_DISABLED",
      });
      console.warn(
        "[Authentication] Warning: DEMO_USER_ENABLED is set to false, but a request was made to login with the DEMO_USER code!",
      );
      return; // Discord will never have "DEMO_USER" as a proper auth code, don't even attempt to pass it on to Discord
    }
    let jwtToken = jwt.sign(
      { discord_id: "demo_user", role: PermissionRole.ARTIFICER },
      SECRET_KEY,
      { expiresIn: "3hr" },
    );
    const fakeUser = {
  ...
  exchangeCodeForToken(req.body.code)
    .then(async (response) => {
      ...
      if (role < PermissionRole.APPRENTICE) {
        res.status(403).json({
          error:
            "This user does not have API access yet. Please contact an Administrator to get access.",
        });
        return;
      }
