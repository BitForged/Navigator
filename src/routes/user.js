...
router.delete(/* /api/user/*/ '/image/:imageId', security.isAuthenticated, (req, res) => {
    db.query('DELETE FROM images WHERE id = ? AND owner_id = ?', [req.params.imageId, req.user.discord_id], (err, _) => {
        if(err) {
            console.error(err);
            res.status(500).json({ message: 'Internal Server Error' });
            return;
...
router.put(/* /api/user/*/ '/image/:imageId/category', security.isAuthenticated, async (req, res) => {
    ...
    if(!image || image.owner_id !== req.user.discord_id) {
        res.status(403).json({message: 'Image does not exist or you do not own it'});
        return;
    }
    ...
    if(categoryId !== null) {
        try {
            ...
            if(!category || category.owner_id !== req.user.discord_id) {
                res.status(403).json({message: 'Category does not exist or you do not own it'});
                return;
            }
        } catch(err) {
            console.error(err);
    ...
    try {
        await database.asyncQuery('UPDATE images SET category_id = ? WHERE id = ? AND owner_id = ?', [categoryId, imageId, req.user.discord_id]);
...
router.get(/* /api/user/*/ '/categories', security.isAuthenticated, async (req, res) => {
    try {
        let categories = await database.getCategoriesForUser(req.user.discord_id);
        // Decode base64 name
        categories = categories.map(category => {
            category.name = Buffer.from(category.name64, 'base64').toString('utf-8');
...
router.post(/* /api/user/*/ '/category', security.isAuthenticated, async (req, res) => {
    ...
    try {
        let category = await database.asyncQuery('INSERT INTO image_categories (owner_id, name64) VALUES (?, ?)', [req.user.discord_id, name64]);
...
router.delete(/* /api/user/*/ '/category/:categoryId', security.isAuthenticated, async (req, res) => {
    if(isNaN(req.params.categoryId)) {
        res.status(400).json({ message: 'Category ID must be a number' });
        return;
    }
    try {
        await database.asyncQuery('DELETE FROM image_categories WHERE id = ? AND owner_id = ?', [req.params.categoryId, req.user.discord_id]);
...
router.patch(/* /api/user/*/ '/category/:categoryId', security.isAuthenticated, async (req, res) => {
    let name = req.body.name;
    if(name === undefined) {
        res.status(400).json({ message: 'missing required fields' });
        return;
    ...
    try {
        await database.asyncQuery('UPDATE image_categories SET name64 = ? WHERE id = ? AND owner_id = ?', [name64, req.params.categoryId, req.user.discord_id]);
        res.status(204).json({ message: 'Category updated' });
    } catch(err) {
        console.error(err);
        res.status(500).json({ message: 'Internal Server Error' });
