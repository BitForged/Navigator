...
axios
  .get(`${constants.SD_API_HOST}/options`)
  .then((response) => {
...
async function checkForProgressAndEmit(task) {
  await new Promise((resolve) => {
    axios
      .get(`${constants.SD_API_HOST}/progress`)
      .then(async (response) => {
...
async function processTxt2ImgTask(task) {
  ...
  await new Promise((resolve) => {
    ...
    if (task.force_hr_fix === true) {
      ...
      if (
        queuedTask.denoising_strength === undefined ||
        queuedTask.denoising_strength === null ||
        queuedTask.denoising_strength === 0.0
      )
    ...
    axios
      .post(`${constants.SD_API_HOST}/txt2img`, queuedTask)
      .then(async (response) => {
