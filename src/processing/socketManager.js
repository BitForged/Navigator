...
function startWsServer() {
  ...
  if (process.env.I_DO_NOT_LIKE_FUN !== null) {
    io.on("connection", (socket) => {
      console.log(
        `Charting a course for client ${socket.id} (${socket.handshake.address}). Steady as she goes!`,
      );
      socket.emit("connected", {
        message: `Welcome aboard! We're navigating uncharted territories together.`,
      });
      socket.on("disconnect", () => {
        console.log(
          `Client ${socket.id} has set sail for distant shores. Until we meet again!`,
        );
      });
    });
  }
