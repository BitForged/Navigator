...
export function isLocalOverrideLora (obj: any): obj is LocalOverrideLora {
    return obj &&
        ...
        obj.trainedWords === undefined ||
            obj.trainedWords !== undefined && Array.isArray(obj.trainedWords) &&
...
export function convertCivitaiAndForgeToNavigatorLora(civitai: CivitAiModelVersion, forge: ForgeLora): NavigatorLora {
    return {
        forge,
        civitai,
        alias: forge.alias,
 ...
 */
export async function getLoraMetadata(lora: ForgeLora): Promise<NavigatorLora | undefined> {
    let hash = ""
    ...
    if(!lora.metadata?.sshs_model_hash) {
        ...
        if(process.env.MODEL_DIR !== undefined) {
            ...
            try {
                ...
                console.error(e)
                console.warn(`Falling back to using alias as the hash for lora: ${lora.name} (Alias: ${lora.alias})`)
                hash = lora.alias
            }
        } else {
            // Can't fall back to calculating the SHA256 hash manually, since we don't know where the model is.
            console.warn("No model directory set, cannot calculate SHA256 hash for lora - falling back to alias as the hash: ", lora.name)
            console.warn("If this is intended, please be sure to register a local override using the alias as the hash. Name: ", lora.name, " -> ", lora.alias)
            hash = lora.alias
        }
    } else {
    ...
    if(metadata) {
        ...
        if(metadata.metadata_provider === ModelMetadataProvider.CIVITAI) {
            ...
            if(metadata.metadata_updated_at.getTime() > Date.now() - CACHE_VALID_RANGE) {
                ...
                console.warn(`Invalid metadata found in database for ${lora.name} - attempting to update again from CivitAI: `, metadataObj)
            }
            if(metadata.updates_disabled) {
                console.warn(`Model ${lora.name} is disabled for updates (and last updated > 7 days ago), returning current cached data.`)
                if (isCivitAiModelVersion(metadataObj)) {
                    return convertCivitaiAndForgeToNavigatorLora(metadataObj, lora)
                } else {
                    console.error("Invalid metadata found in database (updates disabled, cannot resolve): ", metadataObj);
                    return undefined
                }
            }
            ...
            try {
                ...
                console.warn(`[Cache Update Failed] No matching model on CivitAI found for ${lora.name} (${hash}) - was it taken down?`)
            ...
            if(isLocalOverrideLora(obj)) {
                ...
                console.error(`Lora ${lora.name}'s metadata provider is marked as LOCAL, but the metadata is invalid!`)
                return undefined
            }
        }
    } else {
        ...
        try {
            console.log(`No metadata found for lora ${lora.name} (${hash}), attempting to fetch from CivitAI...`)
            ...
            if (resp !== undefined) {
                ...
                console.log(`Successfully fetched metadata for lora ${lora.name} (${hash}) from CivitAI - cached into database.`)
                return convertCivitaiAndForgeToNavigatorLora(resp, lora)
            }
        } catch (e) {
            console.error(`Failed to get metadata for lora ${lora.name} (${hash}) from CivitAI: `, e)
        }
    }
 ...
 */
export async function getModelMetadata(modelHash: string): Promise<ModelMetadata | undefined> {
    const results = await asyncQuery(`SELECT id, hash, model_type, FROM_BASE64(metadata_cache) AS metadata_cache,
                                             metadata_provider, metadata_id, metadata_updated_at, updates_disabled FROM model_metadata WHERE hash = ?`, [modelHash])
    if(results.length > 0) {
...
export async function setModelMetadata(modelHash: string, metadata: ModelMetadata) {
    ...
    if (existingMetadata) {
        ...
        await asyncQuery(
            `UPDATE model_metadata
             ...
             WHERE hash = ?`,
            [metadata.model_type, metadata.metadata_cache, metadata.metadata_provider, metadata.metadata_id, modelHash]
        );
    } else {
        ...
        await asyncQuery(
            `INSERT INTO model_metadata (hash, model_type, metadata_cache, metadata_provider, metadata_id) 
             VALUES (?, ?, TO_BASE64(?), ?, ?)`,
            [modelHash, metadata.model_type, metadata.metadata_cache, metadata.metadata_provider, metadata.metadata_id]
        );
    }
}
