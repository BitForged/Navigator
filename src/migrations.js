...
async function runMigrations() {
    ...
    try {
        const currentMigrations = await database.asyncQuery('SELECT * FROM migrations');
        ...
        console.log("Found existing migrations in database, we're on version: " + highestMigrationRan);
    } catch(err) {
    ...
    if(highestIndex < highestMigrationRan) {
        if(process.env.NAVIGATOR_IGNORE_MIGRATION_MISMATCH === "true" || process.env.NAVIGATOR_IGNORE_MIGRATION_MISMATCH === "1") {
            console.warn("****** WARNING: Migration mismatch bypassed (NAVIGATOR_IGNORE_MIGRATION_MISMATCH set) *******")
            console.warn("Whoa there partner!")
            console.warn("Your database has a migration that Navigator does not know about. You must be a time-traveler!")
            console.warn("Greetings time-traveler, please proceed with caution - what you are attempting to do may attempt in data corruption and unexpected behavior!")
            console.warn("For instructions on how to fix this, unset the `NAVIGATOR_IGNORE_MIGRATION_MISMATCH` environmental variable.")
            console.warn("By proceeding without resolving this issue, you could end up shattering the timeline!")
            console.warn("Don't say we didn't warn you.")
            console.warn("*********************************************************************************************")
        } else {
            console.error("****** ERROR: Database migration inconsistency detected! *******")
            console.error(`Found:    ${highestIndex} < ${highestMigrationRan}`);
            console.error(`Expected: ${highestIndex} >= ${highestMigrationRan}`);
            console.error("Your database has a migration that we do not know about. What?")
            console.error(`The database has a migration version (${highestMigrationRan}) that is higher than Navigator's expected version (${highestIndex}).`)
            console.error("This indicates a potential downgrade or missing migration files.")
            console.error("This can lead to data corruption or unpredictable behavior. Navigator will now exit.")
            console.error(" ")
            console.error("To resolve this issue:")
            console.error("1. Upgrade Navigator to the latest version, or at least the latest version you were previously using.")
            console.error("2. If you intentionally downgraded, restore a database backup to fix the mismatch.")
            console.error("3. Are you a time-traveler? If so, you should return to your original time!")
            console.error("******************************************************************")
            process.exit(2);
        }
    }
...
function warnMissingMigrationsAndExit() {
    ...
    console.error("Navigator cannot find its migration files! Something is definitely not right.")
    console.error("There are a couple of reasons this could happen:")
    console.error("- You have upgraded Navigator to a version that introduced migrations, but did not obtain the `migrations` folder.")
    console.error("- You *do* have a `migrations` folder, but it is empty (this should never be the case).")
    console.error("- Navigator for some reason cannot access the `migrations` folder.")
    console.error("- Or cosmic rays have hit your storage medium, and effectively mangled or deleted the `migrations` folder.")
    console.error(" ")
    console.error("Nonetheless, Navigator's installation is currently in an unexpected state and cannot continue - Navigator will now exit.")
    console.error("Please make sure the `migrations` folder exists, and try again.")
    console.error("***************************************************")
    process.exit(2);
}
