...
module.exports = {
  ...
  getCategoriesForUser(userId) {
    return this.asyncQuery(
      "SELECT * FROM image_categories WHERE owner_id = ?",
      [userId],
    );
  },
  ...
  async getCategoryById(categoryId) {
    let results = await this.asyncQuery(
      "SELECT * FROM image_categories WHERE id = ?",
      [categoryId],
    );
    if (results.length === 0) {
      return null;
  ...
  async getImageById(imageId) {
    let results = await this.asyncQuery("SELECT * FROM images WHERE id = ?", [
      imageId,
    ]);
    if (results.length === 0) {
      return null;
  ...
  async getImageMetadata(imageId) {
    let results = await this.asyncQuery(
      "SELECT category_id, created_at, message_id, owner_id FROM images WHERE id = ?",
      [imageId],
    );
    if (results.length === 0) {
      return null;
