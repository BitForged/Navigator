...
export async function getLorasFromForge(): Promise<ForgeLoraResponse> {
    ...
    try {
        const response: AxiosResponse<ForgeLoraResponse> = await axios.get<ForgeLoraResponse>(`${SD_API_HOST}/loras`)
        ...
        if(isAxiosError(error)) {
            console.error('A network error occurred when requesting Loras: ', error.response?.data)
        } else {
