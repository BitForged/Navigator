...
import {getAvailableStorage, downloadFileToPath, doesFileExist} from '@/storage';
...
thirdPartyRouter.get('/civitai/downloads_enabled', isAuthenticated, async (req, res) => {
    ...
    if (!await civitai.isAuthenticated()) {
        res.json({ downloadsEnabled: false, message: "CivitAI authentication failed" });
        return;
    }
...
thirdPartyRouter.get('/civitai/download/:modelVersionId', isAuthenticated, isArtificer, async (req, res) => {
    ...
    if (storagePercentage < 10) {
        res.status(500).json({message: 'Low storage space', error: 'LOW_STORAGE_SPACE'});
        return
    }
    ...
    try {
        console.log("Downloading model to: ", `${modelFolderPath}/${primaryFile.name}`)
        await downloadFileToPath(`${data.downloadUrl}?token=${civitai.getApiKey()}`, `${modelFolderPath}/${primaryFile.name}`)
        ...
        const refreshEndpointNames = ['refresh-embeddings', 'refresh-checkpoints', 'refresh-loras']
        for(const endpointName of refreshEndpointNames) {
            try {
                const url = `${constants.SD_API_HOST}/${endpointName}`
                await axios.post(url)
            }catch (e) {
                console.error("Failed to request refresh of models from Forge backend: ", e)
            }
        }

        console.log("Successfully requested refresh of models from Forge backend")
    } catch(e) {
        console.error(e);
